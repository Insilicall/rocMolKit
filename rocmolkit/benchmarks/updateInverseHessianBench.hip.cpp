...
int main(int argc, char** argv) {
  try {
    ...
    cudaCheckError(hipEventCreate(&startEvent));
    cudaCheckError(hipEventCreate(&stopEvent));
    ...
    for (int iter = 0; iter < kNumRuns + 1; ++iter) {
      ...
      cudaCheckError(hipEventRecord(startEvent));
      ...
      cudaCheckError(hipEventRecord(stopEvent));
      cudaCheckError(hipEventSynchronize(stopEvent));

      float elapsedMs = 0.0f;
      cudaCheckError(hipEventElapsedTime(&elapsedMs, startEvent, stopEvent));
      if (iter > 0) {
        timings.push_back(elapsedMs);
      }
    }

    cudaCheckError(hipEventDestroy(startEvent));
    cudaCheckError(hipEventDestroy(stopEvent));
