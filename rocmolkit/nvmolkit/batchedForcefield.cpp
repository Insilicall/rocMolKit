...
void throwIfCudaError(hipError_t err, const std::string& context) {
  if (err != hipSuccess) {
    throw std::runtime_error(context + ": " + hipGetErrorString(err));
  }
}
...
template <typename T> std::vector<T> copyDeviceVector(nvMolKit::AsyncDeviceVector<T>& deviceVec) {
  ...
  throwIfCudaError(hipStreamSynchronize(deviceVec.stream()), "copyDeviceVector/sync");
  return hostVec;
}
...
void uploadConformerPositions(const std::vector<RDKit::ROMol*>&    mols,
  ...
  throwIfCudaError(hipStreamSynchronize(positionsDevice.stream()), "uploadConformerPositions/sync");
}
...
class NativeMMFFBatchedForcefield {
 public:
  NativeMMFFBatchedForcefield(const bp::list&                       molecules,
      ...
      : hwOpts_(hwOpts) {
    throwIfCudaError(hipGetDevice(&gpuId_), "MMFF wrapper/hipGetDevice");
  ...
  bp::object minimizeDevice(int maxIters, double gradTol, int targetGpu) {
    ...
    throwIfCudaError(hipMemcpyAsync(positionsDevice_.data(),
                                     dev.positions.data(),
                                     positionsDevice_.size() * sizeof(double),
                                     hipMemcpyDeviceToDevice,
                                     positionsDevice_.stream()),
                     "MMFF minimizeDevice/positions refresh");
    throwIfCudaError(hipStreamSynchronize(positionsDevice_.stream()), "MMFF minimizeDevice/positions refresh sync");
...
class NativeUFFBatchedForcefield {
 public:
  NativeUFFBatchedForcefield(const bp::list&                       molecules,
                             ...
                             const nvMolKit::BatchHardwareOptions& hwOpts)
      : hwOpts_(hwOpts) {
    throwIfCudaError(hipGetDevice(&gpuId_), "UFF wrapper/hipGetDevice");
    mols_             = nvMolKit::extractMolecules(molecules);
    const int numMols = static_cast<int>(mols_.size());
  ...
  bp::object minimizeDevice(int maxIters, double gradTol, int targetGpu) {
    ...
    throwIfCudaError(hipStreamSynchronize(positionsDevice_.stream()), "UFF minimizeDevice/positions refresh sync");
    return nvMolKit::buildOwningDevice3DResult(dev);
  }
