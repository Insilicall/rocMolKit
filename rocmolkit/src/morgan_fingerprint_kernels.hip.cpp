...
__global__ void morganFingerprintKernelBatch(const cuda::std::span<std::uint32_t> atomInvariants,  // mutable
  ...
  using BlockMergeSort = hipcub::BlockMergeSort<AccumTuple, kBlockSize, 1>;
  ...
  for (size_t radius = 0; radius < maxRadius; radius++) {
    ...
    if constexpr (maxAtoms == 128) {
      ...
      if (tile.thread_rank() < 32) {
        ...
        hipcub::WarpMergeSort<AccumTuple, 2, 32>(warp_temp_64[tileId]).Sort(laneItems, AccumTupleLess());
      ...
      __shared__ typename hipcub::WarpMergeSort<AccumTuple, 1, 32>::TempStorage warp_temp_32[tilesPerBlock];
      ...
      hipcub::WarpMergeSort<AccumTuple, 1, 32>(warp_temp_32[tileId]).Sort(laneItem, AccumTupleLess());
