...
class ConsolidatedDeviceBuffer {
 public:
  void allocate(int maxBatchSize, hipStream_t stream);
  void copyFromHost(const PinnedHostBuffer& host, hipStream_t stream);
...
struct GpuExecutor {
  ...
  hipStream_t stream() const { return computeStream.stream(); }
