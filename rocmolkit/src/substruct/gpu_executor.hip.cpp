...
void GpuExecutor::initializeForStream() {
  hipStream_t s         = computeStream.stream();
  hipStream_t recStream = recursiveStream.stream();
...
void ConsolidatedDeviceBuffer::allocate(int maxBatchSize, hipStream_t stream) {
...
void ConsolidatedDeviceBuffer::copyFromHost(const PinnedHostBuffer& host, hipStream_t stream) {
  cudaCheckError(hipMemcpyAsync(data_.data(),
...
void ConsolidatedDeviceBuffer::setStream(hipStream_t stream) {
  data_.setStream(stream);
}
