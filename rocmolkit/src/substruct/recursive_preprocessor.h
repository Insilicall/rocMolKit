...
struct LeafSubpatterns {
   ...
   */
  void syncToDevice(hipStream_t stream);
...
struct RecursiveScratchBuffers {
  ...
  explicit RecursiveScratchBuffers(hipStream_t stream)
      : patternEntries(),
        overflow(),
        labelMatrixBuffer(),
        intermediateBits() {
  ...
  void waitForBuffer(int bufferIdx) {
    if (patternsAtDepthHostCopyPending[bufferIdx]) {
      cudaCheckError(hipEventSynchronize(patternsAtDepthHostCopyDone[bufferIdx].event()));
   ...
   */
  void recordCopy(int bufferIdx, hipStream_t stream) {
    cudaCheckError(hipEventRecord(patternsAtDepthHostCopyDone[bufferIdx].event(), stream));
