...
void LeafSubpatterns::syncToDevice(hipStream_t stream) {
...
void RecursivePatternPreprocessor::syncToDevice(hipStream_t stream) {
  ...
  for (int currentDepth = 0; currentDepth <= maxDepth; ++currentDepth) {
    ...
    if (patternsForDepth.empty()) {
      if (currentDepth < numDepthEvents && depthEvents != nullptr) {
        cudaCheckError(hipEventRecord(depthEvents[currentDepth], stream));
      }
      continue;
    }
