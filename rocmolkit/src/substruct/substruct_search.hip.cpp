...
void uploadAndLaunchMiniBatch(GpuExecutor&                        executor,
  ...
  hipStream_t executorStream     = executor.stream();
  ...
  hipStream_t recursiveStream = executor.recursiveStream.stream();
  ...
  cudaCheckError(hipEventRecord(executor.allocDoneEvent.event(), executorStream));

  ScopedNvtxRange waitAllocRange("Wait: recursiveStream waits for alloc");
  cudaCheckError(hipStreamWaitEvent(recursiveStream, executor.allocDoneEvent.event(), 0));
  waitAllocRange.pop();

  std::array<hipEvent_t, kMaxSmartsNestingDepth> depthEventPtrs;
  ...
  hipStream_t postStream = executor.postRecursionStream.stream();
  cudaCheckError(hipStreamWaitEvent(postStream, executor.allocDoneEvent.event(), 0));
  ...
  for (int depth = 1; depth <= executor.plan.maxPipelineDepthInMiniBatch; ++depth) {
    ...
    cudaCheckError(hipStreamWaitEvent(postStream, depthEventPtrs[depth - 1], 0));
    waitRange.pop();
  ...
  cudaCheckError(hipEventRecord(executor.postRecursionDoneEvent.event(), postStream));

  cudaCheckError(hipEventRecord(executor.recursiveDoneEvent.event(), recursiveStream));
  cudaCheckError(hipStreamWaitEvent(executorStream, executor.recursiveDoneEvent.event(), 0));
  cudaCheckError(hipStreamWaitEvent(executorStream, executor.postRecursionDoneEvent.event(), 0));
}
...
void runnerWorkerPipeline(int                                 workerIdx,
  ...
  try {
    ...
    auto drainOne = [&]() {
      ...
      cudaCheckError(hipEventSynchronize(oldest->copyDoneEvent.event()));
      waitRange.pop();
    ...
    while (true) {
      ...
      cudaCheckError(hipEventRecord(executor->targetsReadyEvent.event(), executor->stream()));
      cudaCheckError(hipStreamWaitEvent(executor->recursiveStream.stream(), executor->targetsReadyEvent.event(), 0));
      cudaCheckError(
        hipStreamWaitEvent(executor->postRecursionStream.stream(), executor->targetsReadyEvent.event(), 0));
...
void runPipelinedSubstructSearch(const std::vector<const RDKit::ROMol*>& targets,
  ...
  int              currentDevice = 0;
  cudaCheckError(hipGetDevice(&currentDevice));
  if (gpuIds.empty()) {
