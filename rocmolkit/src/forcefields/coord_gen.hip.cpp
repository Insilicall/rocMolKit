...
__global__ void projectDistancesToPositions(const int      matrixDim,
  ...
  hiprand_init(seed, idxWithinBatch, 0, &states[idx]);
  ...
  for (int j = 0; j < DIM; j++) {
    ...
    if (eigval == 0.0) {
      ...
      // result.
      positions[DIM * (startAtomIdx + idxWithinBatch) + j] = 1.0 - 2.0 * hiprand_uniform_double(&states[idx]);
    } else {
...
class InitialCoordinateGenerator::Impl {
 ...
 public:
  ...
  void computeInitialCoordinates(double* deviceCoords, const int* deviceAtomStarts, const uint8_t* active) {
    ...
    hipMemcpyAsync(passFail_.data(), solver_.converged(), batchSize, hipMemcpyDeviceToDevice);
    ...
    AsyncDeviceVector<hiprandState> states(numThreads);
