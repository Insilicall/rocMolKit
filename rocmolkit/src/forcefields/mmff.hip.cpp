...
void setStreams(BatchedMolecularDeviceBuffers& molSystemDevice, hipStream_t stream) {
...
hipError_t computeEnergy(BatchedMolecularDeviceBuffers& molSystemDevice,
  ...
  if (err == hipSuccess && contribs.angleTerms.idx1.size() > 0) {
  ...
  if (err == hipSuccess && contribs.bendTerms.idx1.size() > 0) {
  ...
  if (err == hipSuccess && contribs.oopTerms.idx1.size() > 0) {
  ...
  if (err == hipSuccess && contribs.torsionTerms.idx1.size() > 0) {
  ...
  if (err == hipSuccess && contribs.vdwTerms.idx1.size() > 0) {
  ...
  if (err == hipSuccess && contribs.eleTerms.idx1.size() > 0) {
  ...
  if (err == hipSuccess && contribs.distanceConstraintTerms.idx1.size() > 0) {
  ...
  if (err == hipSuccess && contribs.positionConstraintTerms.idx.size() > 0) {
  ...
  if (err == hipSuccess && contribs.angleConstraintTerms.idx1.size() > 0) {
  ...
  if (err == hipSuccess && contribs.torsionConstraintTerms.idx1.size() > 0) {
...
hipError_t computeEnergy(BatchedMolecularDeviceBuffers& molSystemDevice, const double* coords, hipStream_t stream) {
...
hipError_t computeGradients(BatchedMolecularDeviceBuffers& molSystemDevice,
  ...
  if (err == hipSuccess && activeSystemMask != nullptr && molSystemDevice.indices.atomIdxToBatchIdx.size() > 0) {
...
hipError_t computeGradients(BatchedMolecularDeviceBuffers& molSystemDevice, hipStream_t stream) {
...
hipError_t computeGradBlockPerMol(BatchedMolecularDeviceBuffers& molSystemDevice, hipStream_t stream) {
  const auto pointers       = toPointerStruct(molSystemDevice.contribs);
  const auto indices        = toPointerStruct(molSystemDevice.indices);
  const bool hasConstraints = batchHasConstraints(molSystemDevice.contribs);
