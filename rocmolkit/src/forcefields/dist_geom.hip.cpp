...
void setStreams(BatchedMolecularDeviceBuffers& devBuffers, hipStream_t stream) {
...
void setStreams(BatchedMolecular3DDeviceBuffers& devBuffers, hipStream_t stream) {
  devBuffers.energyBuffer.setStream(stream);
  devBuffers.grad.setStream(stream);
  devBuffers.energyOuts.setStream(stream);
...
hipError_t computeEnergy(BatchedMolecularDeviceBuffers& molSystemDevice,
                          double*                        energyOuts,
  ...
  if (err == hipSuccess && contribs.chiralTerms.idx1.size() > 0) {
  ...
  if (err == hipSuccess && contribs.fourthTerms.idx.size() > 0) {
...
hipError_t computeGradients(BatchedMolecularDeviceBuffers& molSystemDevice,
                             double*                        grad,
                             const int*                     ctxAtomStarts,
                             const double*                  ctxPositions,
                             const double                   chiralWeight,
...
hipError_t computeEnergyETK(BatchedMolecular3DDeviceBuffers& molSystemDevice,
                             double*                          energyOuts,
                             const int*                       ctxAtomStarts,
                             const double*                    ctxPositions,
                             const uint8_t*                   activeSystemMask,
  ...
  // Improper torsion terms
  if (err == hipSuccess && (term == ETKTerm::ALL || term == ETKTerm::IMPPROPER_TORSION) &&
  ...
  // 1-2 distance terms
  if (err == hipSuccess && (term == ETKTerm::ALL || term == ETKTerm::PLAIN || term == ETKTerm::DISTANCE_12) &&
  ...
  // 1-3 distance terms
  if (err == hipSuccess && (term == ETKTerm::ALL || term == ETKTerm::PLAIN || term == ETKTerm::DISTANCE_13) &&
  ...
  // 1-3 angle terms
  if (err == hipSuccess && (term == ETKTerm::ALL || term == ETKTerm::PLAIN || term == ETKTerm::ANGLE_13) &&
  ...
  // Long range distance terms
  if (err == hipSuccess && (term == ETKTerm::ALL || term == ETKTerm::PLAIN || term == ETKTerm::LONGDISTANCE) &&
...
hipError_t computeGradientsETK(BatchedMolecular3DDeviceBuffers& molSystemDevice,
                                double*                          grad,
                                const int*                       ctxAtomStarts,
                                const double*                    ctxPositions,
                                const uint8_t*                   activeSystemMask,
...
hipError_t computePlanarEnergy(BatchedMolecular3DDeviceBuffers& molSystemDevice,
                                double*                          energyOuts,
                                const int*                       ctxAtomStarts,
                                const double*                    ctxPositions,
                                const uint8_t*                   activeSystemMask,
  ...
  // Improper torsion terms
  if (err == hipSuccess && contribs.improperTorsionTerms.idx1.size() > 0) {
    err = launchInversionEnergyKernel(contribs.improperTorsionTerms.idx1.size(),
                                      contribs.improperTorsionTerms.idx1.data(),
                                      contribs.improperTorsionTerms.idx2.data(),
                                      contribs.improperTorsionTerms.idx3.data(),
...
hipError_t computeEnergyBlockPerMol(BatchedMolecularDeviceBuffers&             molSystemDevice,
                                     const nvMolKit::AsyncDeviceVector<int>&    ctxAtomStartsDevice,
                                     const nvMolKit::AsyncDeviceVector<double>& ctxPositionsDevice,
                                     const double                               chiralWeight,
                                     const double                               fourthDimWeight,
...
hipError_t computeGradBlockPerMol(BatchedMolecularDeviceBuffers&             molSystemDevice,
                                   const nvMolKit::AsyncDeviceVector<int>&    ctxAtomStartsDevice,
                                   const nvMolKit::AsyncDeviceVector<double>& ctxPositionsDevice,
                                   const double                               chiralWeight,
                                   const double                               fourthDimWeight,
...
hipError_t computeEnergyBlockPerMolETK(BatchedMolecular3DDeviceBuffers&           molSystemDevice,
                                        const nvMolKit::AsyncDeviceVector<int>&    ctxAtomStartsDevice,
                                        const nvMolKit::AsyncDeviceVector<double>& ctxPositionsDevice,
                                        const uint8_t*                             activeThisStage,
                                        const double*                              positions,
...
hipError_t computeGradBlockPerMolETK(BatchedMolecular3DDeviceBuffers&           molSystemDevice,
                                      const nvMolKit::AsyncDeviceVector<int>&    ctxAtomStartsDevice,
                                      const nvMolKit::AsyncDeviceVector<double>& ctxPositionsDevice,
                                      const uint8_t*                             activeThisStage,
                                      hipStream_t                               stream) {
