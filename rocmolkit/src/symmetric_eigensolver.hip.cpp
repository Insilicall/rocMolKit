...
__global__ void batchEigensolverKernel(const int      numEigs,
  ...
  using BlockReduce = hipcub::BlockReduce<double, 256>;
  ...
  hiprandState* localState          = state + systemIdx * matrixDim;

  if (isLoaderThread) {
    hiprand_init(seed, relIdxWithinSystem, 0, &localState[relIdxWithinSystem]);
  }
  ...
  for (int ei = 0; ei < numEigs; ei++) {
    ...
    if (isLoaderThread) {
      v[relIdxWithinSystem] = hiprand_uniform_double(&localState[relIdxWithinSystem]);
    }
...
void launchBatchEigensolverKernel(const int      numEigs,
  ...
  // TODO: reuse.
  AsyncDeviceVector<hiprandState> states(numSystems * matrixDim);
...
class BatchedEigenSolver::Impl {
 public:
  void solve(int            numEigs,
    ...
    cudaCheckError(hipMemsetAsync(converged_.data(), 0, batch_size * sizeof(uint8_t)));
