...
AsyncDeviceVector<FlatBitVect<fpSize>> computeFingerprintsCuImpl(const std::vector<const RDKit::ROMol*>& mols,
  ...
  cudaCheckError(hipMemsetAsync(outputAccumulator.data(), 0, numMols * sizeof(FlatBitVect<fpSize>), stream));
  ...
  for (auto& perThreadBuffer : threadBuffers) {
    ...
    cudaCheckError(hipEventRecord(perThreadBuffer.prevMemcpyDoneEvent.event(), perThreadBuffer.stream.stream()));
  }

  // Ensure output alloc+memset on external stream is visible to per-thread streams
  ScopedCudaEvent outputAllocDone;
  cudaCheckError(hipEventRecord(outputAllocDone.event(), stream));
  for (const auto& perThreadBuffer : threadBuffers) {
    cudaCheckError(hipStreamWaitEvent(perThreadBuffer.stream.stream(), outputAllocDone.event(), 0));
  }
  range1.pop();
...
#pragma omp parallel for num_threads(nThreadsActual) default(none) shared(numThreadsTotal,     \
  ...
  for (size_t i = 0; i < numThreadsTotal; i++) {
    try {
      ...
      while (hipEventQuery(threadCpuBuffers.prevMemcpyDoneEvent.event()) == hipErrorNotReady) {
      ...
      cudaCheckError(hipEventSynchronize(threadCpuBuffers.prevMemcpyDoneEvent.event()));
      range2.pop();
      ...
      if (scopedChunkSize > 0) {
        ...
        hipStream_t    stream       = threadCpuBuffers.stream.stream();
        ...
        cudaCheckError(hipEventRecord(threadCpuBuffers.prevMemcpyDoneEvent.event(), stream));
        rangeMemcpy.pop();
  ...
  for (const auto& buf : threadBuffers) {
    ...
    cudaCheckError(hipEventRecord(workDone.event(), buf.stream.stream()));
    cudaCheckError(hipStreamWaitEvent(stream, workDone.event(), 0));
  }
