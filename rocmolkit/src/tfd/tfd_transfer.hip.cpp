...
void TFDSystemDevice::setStream(hipStream_t stream) {
...
void transferToDevice(const TFDSystemHost& host, TFDSystemDevice& device, hipStream_t stream) {
  ...
  cudaCheckError(hipMemsetAsync(device.dihedralAngles.data(), 0, totalDihedrals * sizeof(float), stream));
  ...
  cudaCheckError(hipMemsetAsync(device.tfdOutput.data(), 0, totalTFDOutputs * sizeof(float), stream));
}
