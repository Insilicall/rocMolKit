...
struct SimilaritiesRotBuffers {
  ...
  [[nodiscard]] hipStream_t streamForThread(int tid) const {
  ...
  [[nodiscard]] hipEvent_t eventForThreadBuffer(int tid, unsigned int bufIdx) const {
    const bool isA = (tid % 2 == 0);
    if (isA) {
...
#pragma omp parallel for num_threads(2) schedule(dynamic) default(shared)
  for (size_t batchIdx = 0; batchIdx < nBatches; ++batchIdx) {
    try {
      ...
      hipStream_t               currentStream = rotBuffers.streamForThread(tid);
      ...
      for (size_t chunkOffset = 0; chunkOffset < batchElemsTotal; chunkOffset += pinnedCapacityDoubles, bufIdx ^= 1U) {
        ...
        hipEvent_t  bufEvent   = rotBuffers.eventForThreadBuffer(tid, bufIdx);
        hipEvent_t  prevEvent  = rotBuffers.eventForThreadBuffer(tid, prevBufIdx);
        ...
        hipEventRecord(bufEvent, currentStream);
        d2hRange.pop();
      ...
      if (hasPrev) {
        ...
        hipEvent_t     lastEvent  = rotBuffers.eventForThreadBuffer(tid, prevBufIdx);
        hipEventSynchronize(lastEvent);
        const size_t resOffset = (startIdx * nFps2) + prevOffset;
      ...
      hipStreamSynchronize(currentStream);
      syncRange.pop();
    } catch (...) {
...
std::vector<double> crossTanimotoSimilarityCPUResult(const cuda::std::span<const std::uint32_t> bitsOneBuffer,
  ...
  return crossSimilarityImpl(
    ...
    [](const cuda::std::span<const std::uint32_t> bufA,
       ...
       int /*tile*/,
       hipStream_t stream) { launchCrossTanimotoSimilarity(bufA, bufB, nElementsPerFp, out, 0, stream); });
}
...
std::vector<double> crossCosineSimilarityCPUResult(const cuda::std::span<const std::uint32_t> bitsOneBuffer,
  ...
  return crossSimilarityImpl(
    ...
    [](const cuda::std::span<const std::uint32_t> bufA,
       ...
       const cuda::std::span<double>              out,
       int /*tile*/,
       hipStream_t stream) { launchCrossCosineSimilarity(bufA, bufB, nElementsPerFp, out, 0, stream); });
}
