...
struct ETKDGContext {
  explicit ETKDGContext(hipStream_t stream = nullptr) : countFinishedThisIteration(0, stream) {}
...
void setStreams(ETKDGContext& ctx, hipStream_t stream);
...
void launchCollectAndFilterFailuresKernel(ETKDGContext& context, int stageId, hipStream_t stream);
...
int launchGetFinishedKernels(ETKDGContext& context, int iteration, int* finishedCountHost, hipStream_t stream);

//! Set initial active filter, based on if a conformer has succeeded in the previous iteration.
void launchSetRunFilterForIterationKernel(ETKDGContext& context, hipStream_t stream);
