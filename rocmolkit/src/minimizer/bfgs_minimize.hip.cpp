...
template <typename T> void setAll(AsyncDeviceVector<T>& vec, const T& value) {
  ...
  const hipStream_t stream      = vec.stream();
  if (numElements == 0) {
    return;
  }
...
void BfgsBatchMinimizer::initialize(const std::vector<int>& atomStartsHost,
  ...
  if (effectiveBackend == BfgsBackend::PER_MOLECULE) {
    ...
    if (activeThisStage) {
      cudaCheckError(hipMemcpyAsync(activeHost_.data(),
                                     activeThisStage,
                                     numSystems * sizeof(uint8_t),
                                     hipMemcpyDeviceToHost,
                                     stream_));
      cudaCheckError(hipStreamSynchronize(stream_));
    }
  ...
  hipcub::DeviceReduce::TransformReduce(nullptr,
                                     temp_storage_bytes,
                                     lineSearchStatus_.data(),
                                     countFinished_.data(),
...
__global__ void setDirectionKernel(const int*    atomStarts,
  ...
  __shared__ hipcub::BlockReduce<double, 128>::TempStorage tempStorage;
  double           blockMax = hipcub::BlockReduce<double, 128>(tempStorage).Reduce(localMax, cubMax());
...
__global__ void scaleGradKernel(const int16_t* statuses,
  ...
  __shared__ hipcub::BlockReduce<double, 128>::TempStorage tempStorage;
  double blockMax = hipcub::BlockReduce<double, 128>(tempStorage).Reduce(maxGrad, cubMax());
...
void BfgsBatchMinimizer::collectDebugData() {
  ...
  cudaCheckError(hipMemcpyAsync(statusesHost.data(),
                                 statuses_.data(),
                                 numSystems_ * sizeof(int16_t),
                                 hipMemcpyDeviceToHost,
                                 stream_));
  cudaCheckError(hipMemcpyAsync(energiesHost.data(),
                                 energyOutsDevice,
...
bool BfgsBatchMinimizer::minimizeWithMMFF(const int                            numIters,
  ...
  hipError_t err = launchBfgsMinimizePerMolKernel(static_cast<int>(activeMolIds_.size()),
  ...
  if (err != hipSuccess) {
    throw std::runtime_error(std::string("Per-molecule BFGS kernel failed: ") + hipGetErrorString(err));
  }
...
bool BfgsBatchMinimizer::minimizeWithETK(const int                                  numIters,
  ...
  hipError_t err = launchBfgsMinimizePerMolKernelETK(static_cast<int>(activeMolIds_.size()),
                                                      activeMolIdsDevice_.data(),
                                                      maxAtomsInBatch_,
                                                      atomStarts.data(),
                                                      hessianStarts_.data(),
  ...
  if (err != hipSuccess) {
    throw std::runtime_error(std::string("Per-molecule BFGS ETK kernel failed: ") + hipGetErrorString(err));
  }
...
bool BfgsBatchMinimizer::minimizeWithDG(const int                                numIters,
  ...
  hipError_t err = launchBfgsMinimizePerMolKernelDG(static_cast<int>(activeMolIds_.size()),
                                                     activeMolIdsDevice_.data(),
                                                     maxAtomsInBatch_,
                                                     atomStarts.data(),
                                                     hessianStarts_.data(),
  ...
  if (err != hipSuccess) {
    throw std::runtime_error(std::string("Per-molecule BFGS DG kernel failed: ") + hipGetErrorString(err));
  }
