...
void appendBatch(const std::vector<ConformerInfo>& batchConformers,
  ...
  const hipStream_t collectorStream = collector.stream;
  const hipStream_t positionsStream = positionsDevice.stream();
  const hipStream_t energiesStream  = energiesDevice.stream();
  const auto         waitOnStream    = [collectorStream](hipStream_t producerStream) {
    if (producerStream == collectorStream) {
      return;
    }
    ScopedCudaEvent ready;
    cudaCheckError(hipEventRecord(ready.event(), producerStream));
    cudaCheckError(hipStreamWaitEvent(collectorStream, ready.event(), 0));
  };
  ...
  cudaCheckError(hipStreamSynchronize(statusesDevice.stream()));
  ...
  cudaCheckError(hipMemcpyAsync(collector.positions.data() + prevPositionsSize,
                                 positionsDevice.data(),
                                 static_cast<size_t>(totalNewAtoms) * 3 * sizeof(double),
                                 hipMemcpyDeviceToDevice,
                                 collector.stream));
  cudaCheckError(hipMemcpyAsync(collector.energies.data() + prevConfCount,
                                 energiesDevice.data(),
  ...
  cudaCheckError(hipMemcpyAsync(collector.converged.data() + prevConfCount,
                                 convergedHost.data(),
  ...
  cudaCheckError(hipStreamSynchronize(collector.stream));
}
...
template <typename T> std::vector<T> downloadToHost(const AsyncDeviceVector<T>& vec) {
  ...
  if (!host.empty()) {
    vec.copyToHost(host);
    cudaCheckError(hipStreamSynchronize(vec.stream()));
  }
  return host;
}
