 ...
 public:
  DeviceTimings() : labels_(DeviceTimingsData::kMaxSections) {
    if constexpr (enableDeviceTimings) {
      cudaCheckError(hipMalloc(&d_data_, sizeof(DeviceTimingsData)));
      reset();
    }
  }
  ...
  ~DeviceTimings() {
    if constexpr (enableDeviceTimings) {
      if (d_data_ != nullptr) {
        cudaCheckErrorNoThrow(hipFree(d_data_));
      }
    }
  }
  ...
  void reset() {
    if constexpr (enableDeviceTimings) {
      cudaCheckError(hipMemset(d_data_, 0, sizeof(DeviceTimingsData)));
    }
  }
  ...
  void print() const {
    if constexpr (enableDeviceTimings) {
      ...
      cudaCheckError(hipMemcpy(&host_data, d_data_, sizeof(DeviceTimingsData), hipMemcpyDeviceToHost));
