...
template <typename T> class PinnedHostVector {
 public:
  ...
  explicit PinnedHostVector(const size_t size) : size_(size) {
    if (size_ > 0) {
      cudaCheckError(hipHostMalloc(&data_, size_ * sizeof(T)));
    }
  }
  ...
  void resize(const size_t newSize) {
    ...
    T* newData = nullptr;
    cudaCheckError(hipHostMalloc(&newData, newSize * sizeof(T)));
  ...
  template <typename U>
  void copyFromDevice(const AsyncDeviceVector<U>& deviceVec, const hipStream_t stream = nullptr) {
    ...
    cudaCheckError(hipMemcpyAsync(data_, deviceVec.data(), size_ * sizeof(T), hipMemcpyDeviceToHost, stream));
  }

  template <typename U> void copyToDevice(AsyncDeviceVector<U>& deviceVec, const hipStream_t stream = nullptr) const {
    static_assert(std::is_same_v<T, U>, "Type mismatch between host and device vectors");
    if (size() != deviceVec.size()) {
      throw std::out_of_range("Size mismatch: host size " + std::to_string(size()) + " != device size " +
                              std::to_string(deviceVec.size()));
    }
    cudaCheckError(hipMemcpyAsync(deviceVec.data(), data_, size_ * sizeof(T), hipMemcpyHostToDevice, stream));
  }
