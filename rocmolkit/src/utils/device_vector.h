...
template <typename T> class AsyncDeviceVector {
 public:
  ...
  void zero() {
    if (size_ > 0) {
      cudaCheckError(hipMemsetAsync(data_, 0, size_ * sizeof(T), stream_));
    }
  }
  ...
  void copyFromHost(const T* hostData, size_t size, size_t firstElementHost = 0, size_t firstElementDevice = 0) {
    ...
    cudaCheckError(hipMemcpyAsync(data_ + firstElementDevice,
  ...
  void copyToHost(T* hostData, size_t size, size_t firstElementHost = 0, size_t firstElementDevice = 0) const {
    ...
    cudaCheckError(hipMemcpyAsync(hostData + firstElementHost,
...
template <typename T> class AsyncDevicePtr {
 public:
  ...
  T*   data() const noexcept { return data_; }
  void set(const T& data) { cudaCheckError(hipMemcpyAsync(data_, &data, sizeof(T), hipMemcpyHostToDevice, stream_)); }
  void get(T& data) const { cudaCheckError(hipMemcpyAsync(&data, data_, sizeof(T), hipMemcpyDeviceToHost, stream_)); }
  void memSet(int value) { cudaCheckError(hipMemsetAsync(data_, value, sizeof(T), stream_)); }
  void setStream(const hipStream_t stream) noexcept { stream_ = stream; }
  hipStream_t stream() const noexcept { return stream_; }
