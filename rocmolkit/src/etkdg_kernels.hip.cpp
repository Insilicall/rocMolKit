...
__global__ void getFinishedKernels(const int      numTerms,
  ...
  using WarpReduce = hipcub::WarpReduce<int16_t>;
...
void launchCollectAndFilterFailuresKernel(ETKDGContext& context, const int stageId, hipStream_t stream) {
...
int launchGetFinishedKernels(ETKDGContext& context, const int iteration, int* finishedCountHost, hipStream_t stream) {
  const int numTerms = context.nTotalSystems;
  int       gridSize = (numTerms + kBlockSize - 1) / kBlockSize;
  ...
  cudaCheckError(hipGetLastError());
  cudaCheckError(hipMemcpyAsync(finishedCountHost,
                                 context.countFinishedThisIteration.data(),
                                 sizeof(int),
