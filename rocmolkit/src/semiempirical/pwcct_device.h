...
inline double heatOfFormationPm6Kcal(double eElec, int nAtoms, const int* atoms,
  ...
  for (int a = 0; a < nAtoms; ++a) {
    ...
    // The bound check also guards the out-of-range elements now reachable via the
    // SCF (e.g. Hg, Z=80 > kPwcctMaxZ) from indexing past the table.
    if (atoms[a] < 0 || atoms[a] > kPwcctMaxZ || kHofRef[atoms[a]] == 0.0) return NAN;
    ref += kHofRef[atoms[a]];
  }
...
NVMOLKIT_HD inline double heatOfFormationPm6KcalAp(double eElec, int nAtoms,
  ...
  for (int a = 0; a < nAtoms; ++a) {
    // uncalibrated / out-of-range element (e.g. Hg, Z=80 > kPwcctMaxZ) -> HoF
    // unavailable, and avoid indexing kHofRef/kPcore past the table.
    if (ap[a].z < 0 || ap[a].z > kPwcctMaxZ || kHofRef[ap[a].z] == 0.0) return NAN;
    ref += kHofRef[ap[a].z];
  }
  for (int i = 0; i < nAtoms - 1; ++i)
    for (int j = i + 1; j < nAtoms; ++j) {
