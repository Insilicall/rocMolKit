...
// d-p overlap block (5 d on A, 3 p on B), qnB = principal qn of B's p shell:
// 2 (jcall 5) or 3 (jcall 6). out is 5*3 row-major [d][p].
NVMOLKIT_HD inline void dpBlockDev(double zd, double zp, double Rb, int qnB,
                                   double ca, double cb, double sa, double sb, double* out) {
  using namespace ovdetail;
  double A[kOvNMax], B[kOvNMax];
  ...
  if (qnB <= 2) {  // d(qn3) - p(qn2), jcall 5
    ...
    s322 = pre
        * (((A[4] - A[2]) * (B[0] - B[2]) + (A[3] - A[5]) * (-B[1] + B[3]))
        ...
        / (32.0 * std::sqrt(6.0));
  } else {  // d(qn3) - p(qn3), jcall 6
    const double pre = std::pow(zp, 3.5) * std::pow(zd, 3.5) * std::pow(Rb, 7);
    s321 = pre
        * ((A[3] * (3 * B[0] - B[2]) + A[4] * (B[1] + B[3]) - A[5] * (B[0] + B[2]) - A[6] * (3 * B[3] - B[1]))
           + (-A[2] * (3 * B[1] - B[3]) - A[3] * (B[2] + B[4]) + A[4] * (B[1] + B[3]) + A[5] * (3 * B[4] - B[2]))
           - (A[1] * (3 * B[2] - B[4]) + A[2] * (B[3] + B[5]) - A[3] * (B[2] + B[4]) - A[4] * (3 * B[5] - B[3]))
           - (-A[0] * (3 * B[3] - B[5]) - A[1] * (B[4] + B[6]) + A[2] * (B[3] + B[5]) + A[3] * (3 * B[6] - B[4])))
        / (192.0 * std::sqrt(15.0));
    s322 = pre
        * (((A[5] - A[3]) * (B[0] - B[2]) + (A[4] - A[6]) * -(B[1] - B[3]))
           + ((A[4] - A[2]) * -(B[1] - B[3]) + (A[3] - A[5]) * (B[2] - B[4]))
           - ((A[3] - A[1]) * (B[2] - B[4]) + (A[2] - A[4]) * -(B[3] - B[5]))
           - ((A[2] - A[0]) * -(B[3] - B[5]) + (A[1] - A[3]) * (B[4] - B[6])))
        / (192.0 * std::sqrt(5.0));
  }
...
NVMOLKIT_HD inline int diatomOverlapDDev(const AtomIntParams& pA, const double coordA[3],
  ...
  if (nB >= 4) {
    double dp[15];
    dpBlockDev(pA.zetaD, pB.zetaP, Rb, pB.qn, ca, cb, sa, sb, dp);
    for (int m = 0; m < 5; ++m)
