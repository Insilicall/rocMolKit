...
NVMOLKIT_HD inline int diatomOverlapSpDev(const AtomIntParams& pA, const double coordA[3],
  ...
  else if (pA.qn == 4 && pB.qn == 3) jcall = 7;    // Br/qn4 + Cl/S (qn3)
  else if (pA.qn == 5 && pB.qn == 3) jcall = 853;  // I/qn5 + Cl/S (qn3)
  else if (pA.qn == 5 && pB.qn == 4) jcall = 9;    // I/qn5 + Br (qn4)
  else {  // unsupported pair
    for (int i = 0; i < nA * nB; ++i) outBlock[i] = 0.0;
    return nA * nB;
  }
  ...
  if (jcall == 2) {
  ...
  } else if (jcall == 7) {  // heavy qn=4 + qn=3 (Br-Cl/Br-S)
    const double s14 = std::sqrt(14.0), s42 = std::sqrt(42.0);
    S111 = std::pow(zsB, 3.5) * std::pow(zsA, 4.5) * std::pow(Rb, 8)
           * (A111[7] * B111[0] + B111[1] * A111[6] - 3.0 * B111[2] * A111[5]
              - 3.0 * B111[3] * A111[4] + 3.0 * A111[3] * B111[4] + 3.0 * B111[5] * A111[2]
              - A111[1] * B111[6] - A111[0] * B111[7]) / (s14 * 2880.0);
    if (nA > 1 && nB > 1) {
      S211 = std::pow(zsB, 3.5) * std::pow(zpA, 4.5) * std::pow(Rb, 8)
             * (A211[7] * B211[1] + B211[0] * A211[6] - 3.0 * B211[3] * A211[5]
                - 3.0 * A211[4] * B211[2] + 3.0 * A211[3] * B211[5] + 3.0 * A211[2] * B211[4]
                - B211[7] * A211[1] - B211[6] * A211[0]) / (960.0 * s42);
      S121 = std::pow(zpB, 3.5) * std::pow(zsA, 4.5) * std::pow(Rb, 8)
             * ((A121[6] * B121[0] - A121[7] * B121[1]) + 2.0 * (A121[5] * B121[1] - A121[6] * B121[2])
                - (A121[4] * B121[2] - A121[5] * B121[3]) - 4.0 * (A121[3] * B121[3] - A121[4] * B121[4])
                - (A121[2] * B121[4] - A121[3] * B121[5]) + 2.0 * (A121[1] * B121[5] - A121[2] * B121[6])
                + (A121[0] * B121[6] - A121[1] * B121[7])) / (960.0 * s42);
      S221 = std::pow(zpB, 3.5) * std::pow(zpA, 4.5) * std::pow(Rb, 8)
             * ((A22[5] * B22[0] - A22[7] * B22[2]) + (A22[4] * B22[1] - A22[6] * B22[3])
                - 2.0 * (A22[3] * B22[2] - A22[5] * B22[4]) - 2.0 * (A22[2] * B22[3] - A22[4] * B22[5])
                + (A22[1] * B22[4] - A22[3] * B22[6]) + (A22[0] * B22[5] - A22[2] * B22[7])) / (960.0 * s14);
      S222 = std::pow(zpB, 3.5) * std::pow(zpA, 4.5) * std::pow(Rb, 8)
             * ((A22[7] - A22[5]) * (B22[0] - B22[2]) + (A22[6] - A22[4]) * (B22[1] - B22[3])
                - 2.0 * (A22[5] - A22[3]) * (B22[2] - B22[4]) - 2.0 * (A22[4] - A22[2]) * (B22[3] - B22[5])
                + (A22[3] - A22[1]) * (B22[4] - B22[6]) + (A22[2] - A22[0]) * (B22[5] - B22[7]))
             / (1920.0 * s14);
    }
  } else if (jcall == 853) {  // heavy qn=5 + qn=3 (I-Cl/I-S)
    const double s35 = std::sqrt(35.0), s105 = std::sqrt(105.0);
    S111 = std::pow(zsB, 3.5) * std::pow(zsA, 5.5) * std::pow(Rb, 9)
           * (A111[8] * B111[0] + 2.0 * B111[1] * A111[7] - 2.0 * B111[2] * A111[6]
              - 6.0 * B111[3] * A111[5] + 6.0 * A111[3] * B111[5] + 2.0 * B111[6] * A111[2]
              - 2.0 * B111[7] * A111[1] - B111[8] * A111[0]) / (s35 * 17280.0);
    if (nA > 1 && nB > 1) {
      S211 = std::pow(zsB, 3.5) * std::pow(zpA, 5.5) * std::pow(Rb, 9)
             * ((A211[7] * B211[0] + A211[8] * B211[1]) - (-A211[6] * B211[1] - A211[7] * B211[2])
                - 3.0 * (A211[5] * B211[2] + A211[6] * B211[3]) + 3.0 * (-A211[4] * B211[3] - A211[5] * B211[4])
                + 3.0 * (A211[3] * B211[4] + A211[4] * B211[5]) - 3.0 * (-A211[2] * B211[5] - A211[3] * B211[6])
                - (A211[1] * B211[6] + A211[2] * B211[7]) + (-A211[0] * B211[7] - A211[1] * B211[8]))
             / (5760.0 * s105);
      S121 = std::pow(zpB, 3.5) * std::pow(zsA, 5.5) * std::pow(Rb, 9)
             * ((A121[7] * B121[0] - A121[8] * B121[1]) + 3.0 * (A121[6] * B121[1] - A121[7] * B121[2])
                + (A121[5] * B121[2] - A121[6] * B121[3]) - 5.0 * (A121[4] * B121[3] - A121[5] * B121[4])
                - 5.0 * (A121[3] * B121[4] - A121[4] * B121[5]) + (A121[2] * B121[5] - A121[3] * B121[6])
                + 3.0 * (A121[1] * B121[6] - A121[2] * B121[7]) + (A121[0] * B121[7] - A121[1] * B121[8]))
             / (5760.0 * s105);
      S221 = std::pow(zpB, 3.5) * std::pow(zpA, 5.5) * std::pow(Rb, 9)
             * ((A22[6] * B22[0] - A22[8] * B22[2]) + 2.0 * (A22[5] * B22[1] - A22[7] * B22[3])
                - (A22[4] * B22[2] - A22[6] * B22[4]) + 4.0 * (A22[3] * B22[3] - A22[5] * B22[5])
                - (A22[2] * B22[4] - A22[4] * B22[6]) + 2.0 * (A22[1] * B22[5] - A22[3] * B22[7])
                + (A22[0] * B22[6] - A22[2] * B22[8])) / (5760.0 * s35);
      S222 = std::pow(zpB, 3.5) * std::pow(zpA, 5.5) * std::pow(Rb, 9)
             * ((A22[8] - A22[6]) * (B22[0] - B22[2]) + 2.0 * (A22[7] - A22[5]) * (B22[1] - B22[3])
                - (A22[6] - A22[4]) * (B22[2] - B22[4]) + 4.0 * (A22[5] - A22[3]) * (B22[3] - B22[5])
                - (A22[4] - A22[2]) * (B22[4] - B22[6]) + 2.0 * (A22[3] - A22[1]) * (B22[5] - B22[7])
                + (A22[2] - A22[0]) * (B22[6] - B22[8])) / (11520.0 * s35);
    }
  } else if (jcall == 9) {  // heavy qn=5 + qn=4 (I-Br)
    const double s10 = std::sqrt(10.0), s30 = std::sqrt(30.0);
    S111 = std::pow(zsB, 4.5) * std::pow(zsA, 5.5) * std::pow(Rb, 10)
           * (A111[9] * B111[0] + B111[1] * A111[8] - 4.0 * B111[2] * A111[7]
              - 4.0 * B111[3] * A111[6] + 6.0 * A111[5] * B111[4] + 6.0 * B111[5] * A111[4]
    ...
    if (nA > 1 && nB > 1) {
      ...
      S121 = std::pow(zpB, 4.5) * std::pow(zsA, 5.5) * std::pow(Rb, 10)
             * ((A121[8] * B121[0] - A121[9] * B121[1]) + 2.0 * (A121[7] * B121[1] - A121[8] * B121[2])
                - 2.0 * (A121[6] * B121[2] - A121[7] * B121[3]) - 6.0 * (A121[5] * B121[3] - A121[6] * B121[4])
                + 6.0 * (A121[3] * B121[5] - A121[4] * B121[6]) + 2.0 * (A121[2] * B121[6] - A121[3] * B121[7])
                - 2.0 * (A121[1] * B121[7] - A121[2] * B121[8]) - (A121[0] * B121[8] - A121[1] * B121[9]))
             / (80640.0 * s30);
      S221 = std::pow(zpB, 4.5) * std::pow(zpA, 5.5) * std::pow(Rb, 10)
             * ((A22[7] * B22[0] - A22[9] * B22[2]) + (A22[6] * B22[1] - A22[8] * B22[3])
                - 3.0 * (A22[5] * B22[2] - A22[7] * B22[4]) - 3.0 * (A22[4] * B22[3] - A22[6] * B22[5])
                + 3.0 * (A22[3] * B22[4] - A22[5] * B22[6]) + 3.0 * (A22[2] * B22[5] - A22[4] * B22[7])
                - (A22[1] * B22[6] - A22[3] * B22[8]) - (A22[0] * B22[7] - A22[2] * B22[9])) / (80640.0 * s10);
      S222 = std::pow(zpB, 4.5) * std::pow(zpA, 5.5) * std::pow(Rb, 10)
             * ((A22[9] - A22[7]) * (B22[0] - B22[2]) + (A22[8] - A22[6]) * (B22[1] - B22[3])
                - 3.0 * (A22[7] - A22[5]) * (B22[2] - B22[4]) - 3.0 * (A22[6] - A22[4]) * (B22[3] - B22[5])
                + 3.0 * (A22[5] - A22[3]) * (B22[4] - B22[6]) + 3.0 * (A22[4] - A22[2]) * (B22[5] - B22[7])
                - (A22[3] - A22[1]) * (B22[6] - B22[8]) - (A22[2] - A22[0]) * (B22[7] - B22[9]))
             / (161280.0 * s10);
    }
  }
  ...
  if (jcall == 3 || jcall == 431 || jcall == 541 || jcall == 651) {
    if (nA > 1)
      for (int k = 0; k < 3; ++k) outBlock[(k + 1) * nB + 0] = S211 * r0[k];
             ...
             || jcall == 8 || jcall == 10 || jcall == 7 || jcall == 853 || jcall == 9) {
    if (nA > 1 && nB > 1) {
      for (int k = 0; k < 3; ++k) {
