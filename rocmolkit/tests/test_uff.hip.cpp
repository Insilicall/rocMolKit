...
double computeEnergyViaForcefield(BatchedMolecularSystemHost& systemHost, AsyncDeviceVector<double>& positionsDevice) {
  ...
  double energy = 0.0;
  CHECK_CUDA_RETURN(hipMemcpy(&energy, energyOuts.data(), sizeof(double), hipMemcpyDeviceToHost));
  return energy;
}
...
TEST_F(UFFGpuTestFixture, BlockPerMolMatchesRDKit) {
  ...
  CHECK_CUDA_RETURN(hipMemcpy(&gotEnergy, systemDevice_.energyOuts.data(), sizeof(double), hipMemcpyDeviceToHost));
