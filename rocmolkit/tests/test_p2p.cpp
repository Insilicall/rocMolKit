...
template <typename T> AsyncDeviceVector<T> deviceVectorFromHost(const std::vector<T>& host, hipStream_t stream) {
...
template <typename T> std::vector<T> hostVectorFromDevice(const AsyncDeviceVector<T>& dev, hipStream_t stream) {
...
TEST(P2P, CopyDeviceToDeviceCrossGpu) {
  ...
  int canAccess = 0;
  cudaCheckError(hipDeviceCanAccessPeer(&canAccess, 0, 1));
  if (canAccess == 0) {
  ...
  {
    const WithDevice withDst(1);
    cudaCheckError(hipMemcpyAsync(result.data(),
                                   dstDev.data(),
                                   src.size() * sizeof(double),
                                   hipMemcpyDeviceToHost,
                                   dstStream->stream()));
    cudaCheckError(hipStreamSynchronize(dstStream->stream()));
  }
  EXPECT_EQ(result, src);
}
...
TEST(P2P, CopyZeroBytesIsNoop) {
  ...
  copyDeviceToDeviceAsync(dstDev.data(),
                          ...
                          /*dstGpu=*/0,
                          stream.stream());
  cudaCheckError(hipStreamSynchronize(stream.stream()));
}
