...
class SyntheticMoleculeBuilder {
 public:
  ...
  TargetMoleculeView buildTargetView(hipStream_t stream) {
  ...
  QueryMoleculeView buildQueryView(hipStream_t stream) {
    d_atomDataPacked_ = AsyncDeviceVector<AtomDataPacked>(numAtoms_, stream);
...
class SubstructAlgosTestBase : public ::testing::Test {
 protected:
  ...
  void populateLabelMatrixOnDevice(const TargetMoleculeView&                                    target,
    ...
    cudaCheckError(hipStreamSynchronize(stream_.stream()));
  }
};
...
TEST_P(VF2AlgoTest, SingleAtomMatch) {
  ...
  cudaCheckError(hipMemcpyAsync(d_labelMatrix.data(),
                                 &h_labelMatrix,
                                 sizeof(LabelStorage),
  ...
  int matchCount    = 0;
  int reportedCount = 0;
  cudaCheckError(hipMemcpy(&matchCount, d_matchCount.data(), sizeof(int), hipMemcpyDeviceToHost));
  cudaCheckError(hipMemcpy(&reportedCount, d_reportedCount.data(), sizeof(int), hipMemcpyDeviceToHost));
...
TEST_F(SubstructAlgosTestBase, GSIPaintMode) {
  ...
  cudaCheckError(hipMemcpy(h_recursiveBits.data(),
                            d_recursiveBits.data(),
...
TEST_F(SubstructAlgosTestBase, GSIPartialsSpillToOverflow) {
  ...
  cudaCheckError(hipMemcpy(&matchCount, d_matchCount.data(), sizeof(int), hipMemcpyDeviceToHost));
  cudaCheckError(hipMemcpy(&overflowFlag, d_overflowFlag.data(), sizeof(uint8_t), hipMemcpyDeviceToHost));
