...
TEST(AsyncDeviceVector, GoodData) {
  nvMolKit::AsyncDeviceVector<int> vec(10);
  cudaCheckError(hipMemset(vec.data(), 0, 10 * sizeof(int)));
  int* data = vec.data();
...
TEST(AsyncDeviceVector, Zero) {
  nvMolKit::AsyncDeviceVector<int> vec(10);
  cudaCheckError(hipMemset(vec.data(), 1, 10 * sizeof(int)));
  vec.zero();
  int* data             = vec.data();
  int  returnedData[10] = {0};
  cudaCheckError(hipMemcpy(returnedData, data, 10 * sizeof(int), hipMemcpyDeviceToHost));
  for (int i = 0; i < 10; i++) {
...
TEST(AsyncDeviceVector, CopyFromHostZero) {
  ...
  int  returnedData[10] = {0};
  hipMemcpy(returnedData, deviceData, 10 * sizeof(int), hipMemcpyDeviceToHost);
  for (int i = 0; i < 10; i++) {
    EXPECT_EQ(returnedData[i], 0);
  }
}
...
TEST(AsyncDeviceVector, CopyFromHost) {
  nvMolKit::AsyncDeviceVector<int> vec(10);
  hipMemset(vec.data(), 0, 10 * sizeof(int));
  ...
  hipMemcpy(returnedData.data(), deviceData, 5 * sizeof(int), hipMemcpyDeviceToHost);
...
TEST(AsyncDeviceVector, CopyToHostZero) {
  ...
  int*                             deviceData = vec.data();
  hipMemset(deviceData, 0, 5 * sizeof(int));
...
TEST(AsyncDeviceVector, CopyToHost) {
  nvMolKit::AsyncDeviceVector<int> vec(10);
  std::vector<int>                 data = {1, 2, 3, 4, 5};
  hipMemcpy(vec.data(), data.data(), 5 * sizeof(int), hipMemcpyHostToDevice);
...
TEST(AsyncDevicePtr, ConstructorAndSet) {
  ...
  int                           result = -1;
  hipMemcpy(&result, ptr.data(), sizeof(int), hipMemcpyDeviceToHost);
  EXPECT_EQ(result, 10);
  ptr.set(20);
  hipMemcpy(&result, ptr.data(), sizeof(int), hipMemcpyDeviceToHost);
  EXPECT_EQ(result, 20);
...
TEST(AsyncDevicePtr, MoveConstructor) {
  ...
  int result = -1;
  hipMemcpy(&result, ptr2.data(), sizeof(int), hipMemcpyDeviceToHost);
  EXPECT_EQ(result, 10);
}
...
TEST(AsyncDeviceVector, Release) {
  ...
  hipMemcpy(result.data(), data, 2 * sizeof(int), hipMemcpyDeviceToHost);
...
class AsyncDeviceVectorResizeTests : public ::testing::Test {
 protected:
  ...
  std::vector<int> getFirstElements() {
    ...
    EXPECT_EQ(hipMemcpy(result.data(), vec.data(), 5 * sizeof(int), hipMemcpyDeviceToHost), hipSuccess);
    return result;
  }
