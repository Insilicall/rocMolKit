...
TEST_P(SymmetricEigenSolverSyntheticTestFixture, SyntheticData) {
  ...
  cudaCheckError(
    hipMemcpy(converged.data(), solver.converged(), numMatrices * sizeof(uint8_t), hipMemcpyDeviceToHost));
...
TEST_P(CoordGenIntegrationTestFixture, MMFF94Data) {
  ...
  cudaCheckError(
    hipMemcpy(converged.data(), coordgen.getPassFail(), mols.size() * sizeof(uint8_t), hipMemcpyDeviceToHost));
