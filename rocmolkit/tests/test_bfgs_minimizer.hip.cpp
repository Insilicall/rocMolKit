...
TEST(BFGSMinimizerTest, AllocationAndIdentity) {
  ...
  ASSERT_EQ(hipMemcpy(hessianHost.data(), hessian, hessianStorageSize * sizeof(double), hipMemcpyDeviceToHost), 0);
...
template <typename T> std::string debugDump(const nvMolKit::AsyncDeviceVector<T>& data, const std::string& name) {
  ...
  hipMemcpy(dataHost.data(), data.data(), data.size() * sizeof(T), hipMemcpyDeviceToHost);
...
class QuarticBatchedForcefield final : public nvMolKit::BatchedForcefield {
 public:
  ...
  hipError_t computeEnergy(double*        energyOuts,
  ...
  hipError_t computeGradients(double*        grad,
                               const double*  positions,
                               const uint8_t* activeSystemMask = nullptr,
                               hipStream_t   stream           = nullptr) override {
    if (activeSystemMask != nullptr) {
...
TEST_F(BFGSMinimizerHarmonicTestFixture, MultipleMinimizeCallsConvergedSystemUnchanged) {
  ...
  hipStreamSynchronize(energyOutsDevice_.stream());
