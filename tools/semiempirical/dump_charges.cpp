...
int main() {
  ...
  while (std::fgets(line, sizeof(line), stdin)) {
    int n = 0, charge = 0, mult = 1;
    if (std::sscanf(line, "%d %d %d", &n, &charge, &mult) < 1 || n <= 0) continue;
    std::vector<int> z(n);
    ...
    bool ok = pm6dCharges(n, z.data(), c.data(), q.data(), &hof, 800, 1e-10, &hofPm6, charge, mult);
